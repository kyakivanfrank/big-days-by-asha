    ...
    <div className="pt-6">
      <nav aria-label="Breadcrumb">
        <ol role="list" className="mx-auto flex max-w-2xl items-center space-x-2 px-4 sm:px-6 lg:max-w-7xl lg:px-8">
          {breadcrumbz.map((breadcrumb, index) => (
            <li key={index}>
          ...
          <li className="text-sm">
            <a href={selectedProduct.href} aria-current="page" className="font-medium text-gray-500 hover:text-gray-600">{selectedProduct.itemName}</a>
          </li>
        </ol>
      </nav>
      ...
      <div className="mx-auto mt-6 max-w-2xl gallery-height sm:px-6 lg:grid lg:max-w-7xl lg:grid-cols-3 lg:grid-rows-2 lg:gap-2 lg:px-8">
        ...
        {selectedProduct.imageSet.map((img, index) => <div key={index} id={index} className={`overflow-hidden lg:block movingIn lg:rounded-lg ${img.css}`} >
      ...
      <div className='lg:hidden sm:px-6 flex mt-0.5 mx-auto'>
        {selectedProduct.imageSet.map((thumb, index) => <div key={index} id={index} className="p-0.5 foc" onClick={() => editArray(index)}>
      ...
      <div className="mx-auto max-w-2xl px-4 pt-10 pb-16 sm:px-6 lg:grid lg:max-w-7xl lg:grid-cols-3 lg:grid-rows-[auto,auto,1fr] lg:gap-x-8 lg:px-8 lg:pt-16 lg:pb-24">
        <div className="lg:col-span-2 lg:border-r lg:border-gray-200 lg:pr-8">
          <h1 className="text-2xl font-bold tracking-tight text-gray-900 sm:text-3xl">{selectedProduct.itemName}</h1>
        </div>
          ...
          <form className="mt-10">
            ...
            <div className="mt-10">
              <div className="flex items-center justify-between">
                <h3 className="text-sm font-medium text-gray-900">Number of Attendees</h3>
                <span className="text-sm font-medium text-indigo-600 hover:text-indigo-500">
                  Size guide
                </span>
              </div>

              <RadioGroup value={attendance} onChange={setattendance} className="mt-4">
                <RadioGroup.Label className="sr-only"> Choose a size </RadioGroup.Label>
                <div className="grid grid-cols-4 gap-4 sm:grid-cols-8 lg:grid-cols-4">
                  {selectedProduct.attendants.map((size) => (
                    <RadioGroup.Option
                      key={size.name}
                      value={size}
                      disabled={!size.choice}
                      className={({ active }) =>
                        classNames(
                          size.choice
